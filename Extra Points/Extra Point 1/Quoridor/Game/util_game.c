...
void LCD_DrawShadow(uint16_t m_x0, uint16_t y0, uint16_t m_x1, uint16_t m_y1, SHADOW_DIRECTION dir, uint16_t color)
{
	switch (dir)
	{
	case NORTH:
...
void Preview_Wall(struct Wall m_wall)
{
	uint16_t m_x0, y0, m_x1, m_y1, discount = 1;

	// Convert Matrix to Spatial position
	if (m_wall.direction == Vertical)
	{
		m_x0 = m_wall.position.x * (SQUARE_SIZE + WALL_WIDTH) + SQUARE_SIZE;
		y0 = m_wall.position.y * (SQUARE_SIZE + WALL_WIDTH);
	}
	else
	{ // if Horizontal
		m_x0 = m_wall.position.x * (SQUARE_SIZE + WALL_WIDTH);
		y0 = m_wall.position.y * (SQUARE_SIZE + WALL_WIDTH) + SQUARE_SIZE;
	}
	...
	{
	case Horizontal:
		m_x1 = m_x0 + WALL_LENGTH;
		m_y1 = y0 + WALL_WIDTH;
		break;
	case Vertical:
		m_x1 = m_x0 + WALL_WIDTH;
		m_y1 = y0 + WALL_LENGTH;
		break;
	default:
	...
	// dont overlap the board checkers
	m_x0 += discount;
	y0 += discount * 2;
	m_x1 -= discount;
	m_y1 -= discount * 2;

	LCD_FillRect(m_x0, y0, m_x1, m_y1, m_wall.color);
}
...
struct Wall Move_Wall(struct Wall m_wall, DIRECTION dir)
{
	struct Vector2D m_vec2d = Get_Relative_Pos(dir);
...
struct Wall Rotate_Wall(struct Wall m_wall, WALL_DIRECTION new_dir)
{
	// remove previous preview
	Remove_Wall(m_wall);

	// update new direction
	m_wall.direction = new_dir;
